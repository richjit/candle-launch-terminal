...
xfer_tokens AS (
  ...
  FROM (
    SELECT block_time AS time, token_mint_address,
      CASE
        WHEN outer_executing_account = 'MoonCVVNZFSYkqNXP6bxHLPL6QQJiMagDL3qcqUQTrG' THEN 'Moon.it'
...
# Pump.fun graduations — simple and fast query.
PUMPFUN_GRADUATIONS_SQL = """\
...
async def ingest_launchpad_stats(engine, http_client: httpx.AsyncClient) -> int:
    ...
    # 2. Graduations (pump.fun — the dominant launchpad)
    rows = await _execute_sql_and_poll(http_client, api_key, PUMPFUN_GRADUATIONS_SQL, timeout_seconds=300)
    if rows:
        db_rows = []
        daily_totals = {}
        for row in rows:
...
async def refresh_launchpad_stats(engine, http_client: httpx.AsyncClient) -> int:
    ...
    if updated:
        logger.info(f"Refreshed {updated} launchpad stat rows from Dune")
    return updated
